...
- Ladder: reps `default … max(12, default)` step 1; holds `default … max(40, default)` step 5. An exercise may override with optional `progression: { ceiling?, step? }` (validator: positive integer step, ceiling ≥ default and reachable in whole steps); exercises that define it use their own ceiling/step, all others use the defaults above.
- Difficulty tiers: `beginner | intermediate | advanced`. `advanced` is a regular, existing tier used for the advanced progression steps (typically the top of a chain). Two separate rules in `constants/workouts.ts`: **default difficulty** (`isDifficultyAllowedForExperience`, `MAX_DEFAULT_DIFFICULTY`: beginner → beginner; some-experience → ≤ intermediate; experienced → all) governs generation and replacement lists; **earned progression** (`isProgressionStepAllowed`, `MAX_PROGRESSION_DIFFICULTY`: beginner/some-experience → intermediate, experienced → advanced) governs level-up traversal — progression is free within the default tier, a user earns one tier beyond it only when stepping out of it, and never above the progression ceiling. Push results: beginner stops at push-ups; some-experience at diamond (no chair) / decline (chair); experienced reaches archer. Only the manual variation picker in Settings → Progression can choose beyond it.
